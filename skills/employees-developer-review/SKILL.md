...
For conventions and checks that apply to code structure (e.g. visual block separation, no if/else, docstring format, docstring consistency with signature), scan the **full body** of each changed function line by line — do not just check whether the pattern exists somewhere, verify it is followed everywhere inside the entity.
