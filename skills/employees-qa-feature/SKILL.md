...
6. If lint errors remain after auto-fix — fix source files manually **in all files** (not just new/changed ones), re-run lint check. Repeat until clean.
...
9. If format errors remain after auto-fix — fix source files manually **in all files** (not just new/changed ones), re-run format check. Repeat until clean.
...
| Skipping lint/format fixes in existing (non-changed) files   | QA's scope includes ALL files — fix lint/format errors in the entire project, not just new/changed files; existing errors will fail CI too                            |
